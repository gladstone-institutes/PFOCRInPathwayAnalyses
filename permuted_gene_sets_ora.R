...
print(GSE_index)
outdir <- "/wynton/group/gladstone/biocore/projects/pfocr_pathway_enrichment_evaluation/permuted_geneset_databases_ora_results_july_2021/"
...
PermuteDatabase <- function(p, GSE_index, path_list, path_annotation,pvalue_results_human_voom, gene_entrez,path_gene_prob, logFC_results_human_voom) {
  ...
  merged=merged[!(is.na(merged$ENTREZID)),]
  ...
  #if(sum(p.adjust(merged$pvalue, "BH") < 0.05, na.rm = TRUE) > 0) {
    res <- clusterProfiler::enricher(
      merged$ENTREZID[ which(p.adjust(merged$pvalue, "BH") < 0.05) ],
    ...
    k <- sapply(res$GeneRatio, function(x) as.numeric(strsplit(x, "/")[[1]][1]))
    n <- sapply(res$GeneRatio, function(x) as.numeric(strsplit(x, "/")[[1]][2]))
    M <- sapply(res$BgRatio, function(x) as.numeric(strsplit(x, "/")[[1]][1]))
    N <- sapply(res$BgRatio, function(x) as.numeric(strsplit(x, "/")[[1]][2]))
    odds_ratio <- (k*(N-M-n+k))/((M-k)*(n-k))
    
    # print(head(res[,1:7]))
    Nsig <- sum(res$qvalue < 0.05, na.rm = TRUE)
    TempRes <- data.frame(qvalues=res$qvalue, odds_ratio)
...
if(sum(p.adjust(merged$pvalue, "BH") < 0.05, na.rm = TRUE) > 0) {
  ##WP
  rsea_results_human_voom_wp <- clusterProfiler::enricher(
    merged$ENTREZID[ which(p.adjust(merged$pvalue, "BH") < 0.05) ],
    universe = merged$ENTREZID,
    pAdjustMethod = "BH",
    pvalueCutoff = 1, #p.adjust cutoff
  ...
  k <- sapply(rsea_results_human_voom_wp$GeneRatio, function(x) as.numeric(strsplit(x, "/")[[1]][1]))
  n <- sapply(rsea_results_human_voom_wp$GeneRatio, function(x) as.numeric(strsplit(x, "/")[[1]][2]))
  M <- sapply(rsea_results_human_voom_wp$BgRatio, function(x) as.numeric(strsplit(x, "/")[[1]][1]))
  N <- sapply(rsea_results_human_voom_wp$BgRatio, function(x) as.numeric(strsplit(x, "/")[[1]][2]))
  odds_ratio <- (k*(N-M-n+k))/((M-k)*(n-k))
  
  
  TempRes <- data.frame(qvalues=rsea_results_human_voom_wp$qvalue, odds_ratio)
  TempRes %<>% filter(qvalues < 0.05)
  TDP_bound_90 <- TempRes %>%
    .$odds_ratio %>%
    quantile(.,c(0.5, 0.75, 0.9, 0.95, 0.99), na.rm=TRUE)
  oTDP_bound_90_wp <- TDP_bound_90
  oNsig_wp <- sum(rsea_results_human_voom_wp$qvalue < 0.05, na.rm = TRUE)
  oTDP_bound_90_wp <- TDP_bound_90
  print(oNsig_wp)
  ...
  ##GO
  rsea_results_human_voom_go <- clusterProfiler::enricher(
    merged$ENTREZID[ which(p.adjust(merged$pvalue, "BH") < 0.05) ],
    universe = merged$ENTREZID,
    pAdjustMethod = "BH",
    pvalueCutoff = 1, #p.adjust cutoff
  ...
  k <- sapply(rsea_results_human_voom_go$GeneRatio, function(x) as.numeric(strsplit(x, "/")[[1]][1]))
  n <- sapply(rsea_results_human_voom_go$GeneRatio, function(x) as.numeric(strsplit(x, "/")[[1]][2]))
  M <- sapply(rsea_results_human_voom_go$BgRatio, function(x) as.numeric(strsplit(x, "/")[[1]][1]))
  N <- sapply(rsea_results_human_voom_go$BgRatio, function(x) as.numeric(strsplit(x, "/")[[1]][2]))
  odds_ratio <- (k*(N-M-n+k))/((M-k)*(n-k))
  
  TempRes <- data.frame(qvalues=rsea_results_human_voom_go$qvalue, odds_ratio)
  TempRes %<>% filter(qvalues < 0.05)
  TDP_bound_90 <- TempRes %>%
    .$odds_ratio %>%
    quantile(.,c(0.5, 0.75, 0.9, 0.95, 0.99), na.rm=TRUE)
  ...
  oNsig_go <- sum(rsea_results_human_voom_go$qvalue < 0.05, na.rm = TRUE)
  print(oNsig_go)
  ...
  ##PFCOR-3sets
  rsea_results_human_voom_pfocr <- clusterProfiler::enricher(
    merged$ENTREZID[ which(p.adjust(merged$pvalue, "BH") < 0.05) ],
    universe = merged$ENTREZID,
    pAdjustMethod = "BH",
    pvalueCutoff = 1, #p.adjust cutoff
  ...
  k <- sapply(rsea_results_human_voom_pfocr$GeneRatio, function(x) as.numeric(strsplit(x, "/")[[1]][1]))
  n <- sapply(rsea_results_human_voom_pfocr$GeneRatio, function(x) as.numeric(strsplit(x, "/")[[1]][2]))
  M <- sapply(rsea_results_human_voom_pfocr$BgRatio, function(x) as.numeric(strsplit(x, "/")[[1]][1]))
  N <- sapply(rsea_results_human_voom_pfocr$BgRatio, function(x) as.numeric(strsplit(x, "/")[[1]][2]))
  odds_ratio <- (k*(N-M-n+k))/((M-k)*(n-k))
  
 
  TempRes <- data.frame(qvalues=rsea_results_human_voom_pfocr$qvalue, odds_ratio)
  TempRes %<>% filter(qvalues < 0.05)
  TDP_bound_90 <- TempRes %>%
    .$odds_ratio %>%
    quantile(.,c(0.5, 0.75, 0.9, 0.95, 0.99), na.rm=TRUE)
  ...
  oNsig_pfocr <- sum(rsea_results_human_voom_pfocr$qvalue < 0.05, na.rm = TRUE)
  print(oNsig_pfocr)
  ...
  k <- sapply(rsea_results_human_voom_pfocr_miny$GeneRatio, function(x) as.numeric(strsplit(x, "/")[[1]][1]))
  n <- sapply(rsea_results_human_voom_pfocr_miny$GeneRatio, function(x) as.numeric(strsplit(x, "/")[[1]][2]))
  M <- sapply(rsea_results_human_voom_pfocr_miny$BgRatio, function(x) as.numeric(strsplit(x, "/")[[1]][1]))
  N <- sapply(rsea_results_human_voom_pfocr_miny$BgRatio, function(x) as.numeric(strsplit(x, "/")[[1]][2]))
  odds_ratio <- (k*(N-M-n+k))/((M-k)*(n-k))
  

  TempRes <- data.frame(qvalues=rsea_results_human_voom_pfocr_miny$qvalue, odds_ratio)
  TempRes %<>% filter(qvalues < 0.05)
  TDP_bound_90 <- TempRes %>%
    .$odds_ratio %>%
    quantile(.,c(0.5, 0.75, 0.9, 0.95, 0.99), na.rm=TRUE)
  ...
  oNsig_pfocr_miny <- sum(rsea_results_human_voom_pfocr_miny$qvalue < 0.05, na.rm = TRUE)
...
if(sum(p.adjust(merged$pvalue, "BH") < 0.05, na.rm = TRUE) == 0) {
  GSE_index_2_100_perm_res <- list(oNsig_wp=NA, rNsig_wp=NA, oNsig_go=NA, rNsig_go=NA, oNsig_pfocr=NA, rNsig_pfcor=NA, oNsig_pfocr_miny=NA, rNsig_pfcor_miny=NA, oTDP_bound_90_wp=NA, oTDP_bound_90_go=NA, oTDP_bound_90_pfocr=NA, oTDP_bound_90_pfocr_miny=NA)
}
